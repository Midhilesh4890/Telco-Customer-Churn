...
def load_data():
    try:
        df = pd.read_csv("churnbigml80.csv")
...
def show_financial_impact(df, metrics):
    ...
    with col1:
        st.markdown(f"""<div class="roi-card"><h3>Current Annual Loss</h3><div class="metric-highlight">${metrics['annual_churn_loss']:,.0f}</div><p>Revenue lost to churn</p></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="roi-card"><h3>Retention Opportunity</h3><div class="metric-highlight">${metrics['annual_churn_loss'] * 0.3:,.0f}</div><p>Potential revenue protection</p></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="roi-card"><h3>Implementation Investment</h3><div class="metric-highlight">$800K</div><p>Total program cost</p></div>""", unsafe_allow_html=True)
    ...
    with col1:
        roi_data = {
            'Metric': ['Total Investment', 'Annual Revenue Protection', 'Net Annual Benefit', 'ROI Percentage', 'Payback Period'],
            'Value': [
                f'-${investment:,.0f}',
    ...
    with col2:
        ...
        fig.add_trace(go.Scatter(name='Net Benefit', x=years, y=net_benefit, mode='lines+markers', line=dict(color='blue', width=3), yaxis='y2'))
        fig.update_layout(title='3-Year Financial Projection ($000s)', xaxis_title='Year', yaxis_title='Amount ($000s)', yaxis2=dict(title='Net Benefit ($000s)', overlaying='y', side='right'), barmode='group')
